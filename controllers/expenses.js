		...
		if (err) {
			...
			if (req.file) {
				cloudUrl
					.imageUrl(req.file)
					.then((expimgUrl) => {
						Expenses.findByIdAndUpdate(expData._id, {
							$set: { userId: userID, expense_image: expimgUrl },
						}).exec((err, updateExp) => {
							if (err) {
								res.send({ error: 'user id is not add in expense' });
							} else {
								expenses_category.updateOne(
									{ expense_category_type: req.body.category },
									{ $push: { expenses: updateExp } },
									(err, updatedata) => {
										if (err) {
											res.send({ error: 'expense is not add in category' });
										} else {
											res.send({
												msg: 'expense is create with image successfully',
												exp: updateExp,
											});
										}
									}
						...
						expenses_category.updateOne(
							...
							(err, updatedata) => {
								if (err) {
									...
									res.send({
										msg: 'expense is create successfully',
										exp: updateExp,
									});
								}
							}
...
exports.update = (req, res) => {
	Expenses.findByIdAndUpdate(req.params.expenseId, req.body).exec(
		(err, expUpdate) => {
			if (err) {
				...
				if (req.file) {
					cloudUrl
						.imageUrl(req.file)
						.then((expimgUrl) => {
							Expenses.findByIdAndUpdate(expUpdate._id, {
								$set: { expense_image: expimgUrl },
							}).exec((err, updateExp) => {
								if (err) {
