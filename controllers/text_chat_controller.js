          ...
          textMsg.save(async (err, data) => {
            if (err) {
              ...
              await user.findOneAndUpdate(
                ...
                { $set: { textCredit: remainingCredit } }
              );
              res.send({
                textMessage: data,
                success: true,
...
exports.seenContactTextMessages = (req, res) => {
  textContact
    .updateOne({ uid: req.params.contact }, req.body)
...
exports.notification = async (req, res) => {
  let push_notification = (firebase_token, text_msg) => {
    const token =
      "dsfJhffJsjilteAOQGWkBj:APA91bHTMgmPP0UQDAYC85J1fRQRruuVVaBwQHlEVIrNmJiqTXubwcuDC_Pc5_zNC1do72eRbVmwUhirV4F1B7ftjM4vtt-jat8TDIULsOv91-Xq3I_yo0ESR9JrSbEqFU85R98fslfM";
    const msg = {};
    const notification = {};
    const webpush = {};
    ...
    notification.icon =
      "http://www.mymember.com/static/media/logo.940eab8a.png";
  ...
  if (userData) {
    ...
    if (result.msg) {
      res.json({
        success: true,
        msg: "notification send success",
        data: result.resp,
      });
    } else {
      res.json({
        success: false,
        msg: "notification not send",
        data: result.resp,
      });
    }
  }
